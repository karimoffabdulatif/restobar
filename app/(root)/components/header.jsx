...
export default function Header() {
	...
	// ✅ Modal 30 sekunddan keyin avtomatik yopiladi
	useEffect(() => {
		if (isNewsModalOpen) {
	...
	return (
		<>
									...
									<li key={name} className="relative">
										{href ? (
											...
											<button
												...
												className="relative inline-block px-1 transition duration-300 hover:scale-110 text-white hover:text-amber-500"
											>
												{name}
							...
							{[
								...
								<li key={href}>
									<Link href={href} onClick={() => setIsSidebarOpen(false)}>
										<span className={`block text-xl px-4 py-2 rounded-md transition ${pathname === href ? 'text-amber-500' : 'text-white hover:text-amber-500'}`}>
											{name}
										</span>
									</Link>
								</li>
			...
			{/* ✅ News modal with 30s progress */}
			{isNewsModalOpen && (
				<div className="fixed inset-0 z-[1000]  bg-black/80 flex items-center justify-center">
					<div className="relative w-[90%] py-2 sm:w-[700px] h-[650px] bg-black rounded-xl overflow-hidden shadow-lg">

						{/* Progress bar */}
						<div className="absolute top-0 left-0 w-full h-[5px] bg-white/30">
							<div className="h-full bg-amber-500 animate-storyProgress"></div>
						</div>

						{/* Story image */}
						<div
							className="w-full h-full bg-cover bg-center"
							style={{ backgroundImage: "url('/hookah.jpg')" }}
						></div>

						{/* Close button */}
						<button
							onClick={() => setIsNewsModalOpen(false)}
							className="absolute top-3 right-4 text-white text-2xl font-bold hover:text-amber-500"
						>
							✕
						</button>
						...
						<div className="absolute top-3 left-4 flex items-center gap-2 text-white text-sm">
							...
							<div className="w-10 h-10 rounded-full overflow-hidden border border-white/20">
								<Image
									src="/mobile-logo.png" // <== LOGO RASMI MANZILINI SHU YERDA KO‘RSATING
									alt="Logo"
									width={32}
									height={32}
							...
							<span>Agora restobar • 2 soat oldin</span>
						</div>
